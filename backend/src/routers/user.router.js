...
router.post(
  ...
  auth,
  handler(async (req, res) => {
    const { type, address, landmark, addressLatLng } = req.body;
    const user = await UserModel.findById(req.user.id);
...
// Set default address
router.put(
  '/address/default/:index',
  auth,
  handler(async (req, res) => {
    const user = await UserModel.findById(req.user.id);
    user.defaultAddress = parseInt(req.params.index);
    await user.save();
    res.send(user.defaultAddress);
  })
);
...
router.put(
  ...
  handler(async (req, res) => {
    ...
    await user.save();
    res.send(user.favoriteRestaurants);
  })
);
